...
def groq_inference(system_message: str, model_name: str, rule_name: str, sales_deck: str) -> typing.Optional[str]:
    ...
    You MUST provide an output in JSON representation with the following fields:
    "rule_name",
    "label",
    "part",
    "suggestion"
    ...
    try:
        ...
        response = client.chat.completions.create(
            messages=[
                ...
                {
                    "role": "user",
                    "content": f"""The text to modify is: {transcript}.
                    Update the specific sections, using the corresponding suggestions from this list items {partes_and_suggestions_to_follow} in the given order.
                    Ensure that the modifications are applied to each section in sequence, keeping the rest of the text unchanged.""",
                }
            ],
            model=model,
            response_format={"type": "json_object"},
        ...
        logger.info(f"Response: {result}")
        try:
            generated_result = json.loads(result)
            print(generated_result)
            return generated_result['optimized_transcript']
        except json.JSONDecodeError:
            logger.error("Invalid JSON output string")
            raise
